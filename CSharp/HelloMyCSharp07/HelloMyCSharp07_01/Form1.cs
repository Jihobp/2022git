...
namespace HelloMyCSharp07_01
{
    public partial class Form1 : Form
    {
        public Form1()
        {
            ...
            //money는 static 변수라서 자식 클래스에서든 부모 클래스 에서든 공통적으로 접근 됨 //공유해서 씀/
            Console.WriteLine(MyParent.money);
            Console.WriteLine(MyChild.money);

            MyChild.openDoor(); //부모 클래스에서의 메소드 그대로 호출 가능 (static이 붙어있는 메소드는 ㅇㅇ. 찍고 바로 쓸 수 있음.)
            ...
            new MyChild{age = 3, name = "이동순"} }; //list는 parent타입인데 child 타입의 인스턴스도 들어간다.  동순이 인스턴스 추가!!! 
            ...
            foreach (var item in family)
            {
                ...
                if (temp != null) //item이 MyParent라면 null 값.
                {
                    temp.study();
                    temp.playgame();
                    //1억 1만원을 두번 더함. // 7999+10001+10001 = 28001
