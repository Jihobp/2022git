...
namespace HelloMyLastCSharp
{
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            ...
            //람다
            // DataGridViewCellEventHandler 이벤트를 함수를 통해 저장.
            ...
            DataGridViewCellEventHandler dgclick2 = userDataGridViewCellClick; //함수생성
            dataGridView2.CellClick += dgclick2; //함수를 이벤트에 저장.
        }

        private void userDataGridViewCellClick(object s, DataGridViewCellEventArgs e)
        {
            //이벤트를 발생시킨 주체 = s
            DataGridView thisGridView = (s as DataGridView); //as 키워드 이용해서 형변환해서 찾을 수 있다. 
            //ㄴ > ????????????????????????????????????????????????????????? 
            //object 타입을 형변환. 이벤트를 발생시키는 주체. 변경되는 녀석. 
            //datagridView 자체 .object가 모든 클래스의 조상이라 하위 클래스로 형변환가능.
            //그걸 datagridview 로넣은거.  간단히, 클릭한걸 DataGridView로 형변환시켜서 변수에 넣어주는 거에요 sr axiom
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                try
                {
                    ...
                    {
                        /* User u = DataManager.Users.Single
                             (x => x.Id.ToString() == textBox3.Text); 갖고올 필요 X, */
