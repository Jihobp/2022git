class SS{ //	1.(CLASS)
	...
	public void setNum(int n) {// 생성자에 저장해주기때문에 고조체에 함수생성해서 넣어준다.
		this.num=n;}
...
public class Java_Ts_Quest01 {
	...
	//	3. 객체 배열
	SS stu[]=new SS[3]; //배열 선언, 객체 생성 아직 x.
	stu[0]=new SS("이동준", 2009038033, "남");
	stu[1]=new SS("이제영", 2007012034, "여");
	stu[2]=new SS("박준현", 2002012034, "남");
	//s[0].getNum(); //반환?
	...
	//6. 양식대로 출력.
	for(int i=0; i<3;i++) {
		System.out.printf("이름: %s\n 학번: %d\n 성별: %s \n",stu[i].getName(),stu[i].getNumber(),stu[i].getGender());
		//printf에는 + 적우면 안된다. !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		//println에는 , 적으면 안된다. !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		//print 에는 아무것도 적으면 안된다!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
