...
int main()
{
	...
	//숫자 2 - 보
		...
		if (rcp == 0)
			printf("가위");
		else if (rcp == 1) //else 떼어버리면=if 가위출력하고 기타도 같이 출력함. if가 아닌 else 라서??????????????????????????????? QQQQQQQ?????
			printf("바위");
		else if (rcp == 2) //else 떼어버리면=if 가위출력하고 기타도 같이 출력함. 
			printf("보");
		else
			printf("기타");
		//default - if else의 else : 만족하지 않는 것.
		...
		//break;를 만나면 switch 문을 끝냄.
		//break를 만나지 않으면 switch문이 안끝나고 다음 경우의 수를 계속 띄운다.
		...
		//가위바위보 경우의 수는 총 아홉개. if로 코드 짜면 엄청 난해함...
		//switch로는 좀더 simple 하게 가능.
		// Q. switch?????????????? if???????????? 차이 좀더..........................
