...
<%! String strName, strID, strPW, strPhone, strGender; %> <!-- 변수선언 ..ㄷ -->
...
try {//db위치, 스키마이름, 인코딩 등
	...
	//session에서 받은 ID이용해서 조회할 것.
	String strQuery = 
	"select * from member where id= '"+strID+"'";
	rs = stmt.executeQuery(strQuery);
	rs.next(); //한줄 읽어오기 (어짜피 id기준이면 1개만 조회됨)
...
<form action="ModifyOk" method="post">
	이름 : <input type="text" name="name" value="<%=strName %>"/><br /> <!--  value==strName % 변수를 넣어준것 -->
