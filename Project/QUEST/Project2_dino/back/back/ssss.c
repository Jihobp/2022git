...
boolean isCollision(const int treeX, const int holemanY)
{
	//트리의 X가 공룡의 몸체쪽에 있을때,
	//공룡의 높이가 충분하지 않다면 충돌로 처리
	GotoXY(0, 0);
	//printf("treeX : %d, holemanY : %d", treeX, holemanY); //이런식으로 적절한 X, Y를 찾습니다.
	...
	while (1)		//(v2.0) 게임 루프
	{
		...
		while (1)	//한 판에 대한 루프
		{
			...
			if (holemanY < HOLEMAN_BOTTOM_Y)
				...
				DrawHoleman(holemanY);		//draw dino
